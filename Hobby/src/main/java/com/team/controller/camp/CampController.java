...
public class CampController {
	 ...
	 * "vftwTJlsPgy8Zr4yGFWel%2B0e54Ai6wRKua4w26fGz03gC5HqPtaQzeqWerg0kCB8f%2FpRosqM4JRLUq5Xvjel9A%3D%3D";
	 */
	...
	public List<CampVO> getCampList(@RequestParam(defaultValue = "1") int pageNo) throws URISyntaxException {
		...
		String apiUrl = targetUrl + "/basedList" + "?numOfRows=" + numOfRows + "&pageNo=" + pageNo + "&MobileOS="
				+ mobileOS + "&MobileApp=" + mobileApp + "&serviceKey=" + serviceKey + "&_type=json";
	...
	public List<CampVO> getSearchList(@RequestParam(defaultValue = "1") int pageNo, @RequestParam String keyword)
		...
		String apiUrl = targetUrl + "/searchList" + "?numOfRows=" + numOfRows + "&pageNo=" + pageNo + "&MobileOS="
	...
	public ModelAndView getCampDetail(@RequestParam String keyword, HttpServletRequest request)
		...
		/*
		 * facltNm : 야영장명 lineIntro : 한줄소개 firstImageUrl : 이미지 addr1 : 전체 주소 doNm :
		 * 주소(시,도) tel : 전화번호 operDeCl : 운영일 lctCl : 캠핑장 환경 (ex: 산, 숲) induty : 캠핑장 유형
		 * (ex: 자동차 야영장) posblFcltyCl : 주변이용가능시설 mapX : 경도(X) mapY : 위도(Y) featureNm :
		 * 소개글
		 */
